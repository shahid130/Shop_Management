...
create or replace package body total_profit_pack as 
    ...
    profitPerProduct int;
    begin 
        for row in ((select product_purchase_rate,product_sales_rate,total_quantity from Product1) UNION (select product_purchase_rate,product_sales_rate,total_quantity from Product2@site2)) loop
            profitPerProduct:=(row.product_sales_rate-row.product_purchase_rate)*row.total_quantity;
            net_profit:=net_profit+profitPerProduct;
        end loop;
